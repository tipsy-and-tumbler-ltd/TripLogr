...
//Rare exceptions to indicate a bug in the _triplogr code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_triplogr = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_triplogr]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.triplogr.module"
